...
async def get_shortlink(chat_id, link, is_active=False):
    settings = await db.get_settings(chat_id)
    if 'shortlink' in settings.keys() and is_active == True:
        URL = settings['shortlink']
    else:
        URL = SHORTLINK_URL
    if 'shortlink_api' in settings.keys() and is_active == True:
